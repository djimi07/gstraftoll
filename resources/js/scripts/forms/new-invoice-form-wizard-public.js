$(function () {
    ...
    $.ajax(
        {
            url: GsTraftoll.url('v1/get-revenue-heads'),
            success: function (result) {
                // console.log(result);

                if ("data" in result == true) {

                    $revenueHeads.empty(); // remove old options
                    $revenueHeads.append($("<option value=''> --Select Revenue Head---</option>"));

                    $.each(result.data, function (index, lga) {
                        $revenueHeads.append($("<option value=''> --Select---</option>")
                            .attr("value", lga.revenueCode).text(lga.name + ' [' + (lga.revenueCode) + ']'));
                    });
            ...
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                $lgaSelect.empty();
                $lgaSelect.html($("<option value=''>" + textStatus.toUpperCase() + " : " + errorThrown.toUpperCase() + "---</option>"));
                $('.btn-next').prop('disabled', true);
    ...
    $revenueHeads.change(function (e) {

        $.ajax(
            {
                url: GsTraftoll.url('v1/get-revenue-items?code=' + $revenueHeads.val()),
                success: function (result) {
                    // console.log(result);
                    ...
                    if ("data" in result == true) {
                        ...
                        $.each(result.data.revenueItems, function (index, lga) {
                    ...
                    $.post({
                        ...
                        success: function (response) {
                            ...
                            if (response.status == 201) {
                                ...
                                window.setTimeout(gotoUsers, 4000);
